...
> ⚠️ **Chart is June-era and now out of sync with the tables** — it is hand-made, not generated by
> `gen_charts.py`, and the numbers in [FREE-ACCESS.md](./FREE-ACCESS.md) changed since (DeepSeek
> peak/off-peak pricing, Cerebras trial credit, Groq limits). Trust the table, not this PNG, until
> it is regenerated from live data.
